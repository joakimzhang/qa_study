...
#   socket_con.recevieResult()
    ...
    if options.std=='DVBS':
        ...
        if re.search('Para Name Error',res):
            print 'Code Rate is failed for setting 2/3, the code rate will use default value'
            socket_con.closeSocket()
            return -10
...
#            socket_con.sendCmd('chpara CarrierMode %d' %options.carriermode)
#            res=socket_con.recevieResult()
