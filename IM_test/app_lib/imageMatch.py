...
def detect_match(src_image, image, noise_threshold=None, match_parameters=None):
    ...
    if match_parameters is None:
        match_parameters = parseConfig.MatchParameters()
...
def _match(image, template, match_parameters, template_name):
    ...
    if _debug_level > 1:
        ...
        cv2.rectangle(
            ...
            thickness=1)
        _log_image(
            source_with_roi, "source_with_roi", "match-debug/detect_match")
...
def _find_match(image, template, match_parameters):
    ...
    log = functools.partial(_log_image, directory="match-debug/detect_match")
    log(image, "source")
    log(template, "template")
    ddebug("Original image %s, template %s" % (image.shape, template.shape))

    levels = parseConfig.get_config("match", "pyramid_levels", type_=int)
    if levels <= 0:
...
if __name__ == '__main__':
    ...
    image_dir = os.path.join(main_dir, 'VideoCapture', 'SnapPictures')
    template_dir = os.path.join(main_dir, 'caseInfo', 'Case_pic')
    match_parameters = parseConfig.MatchParameters()
    src_image = ['20150103-165002_1.bmp']
    template = ['1111_infobar_1.bmp']
    detect_match('messi.jpg', 'template.jpg', match_parameters=match_parameters)